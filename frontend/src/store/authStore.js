        ...
        try {
          ...
          if (userJson) {
            try {
              ...
              console.warn('⚠️ Corrupted user data in localStorage. Clearing stored session.');
              localStorage.removeItem('token');
              localStorage.removeItem('user');
              localStorage.removeItem('role');
          ...
          if (hasValidAuth) {
            api.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            console.log('🔐 [authStore] Auth initialized with valid session.');
            ...
            localStorage.removeItem('user');
            localStorage.removeItem('role');
            set({ token: null, user: null, refreshToken: null, isAuthenticated: false, isLoading: false, error: null });
          }
        } catch (err) {
