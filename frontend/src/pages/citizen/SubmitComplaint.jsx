...
export default function SubmitComplaint() {
  ...
  const processImageWithAI = async (file) => {
    ...
    try {
      ...
      if (analysis.is_complaint !== false && analysis.category !== 'Other') {
        const detectedCategory = analysis.mappedCategory || 'civic_issue';
        const detectedSubcategory = analysis.mappedSubcategory || 'road_damage';
        const autoDescription = analysis.description || analysis.analysis || analysis.summary || 'Civic issue detected from photo upload.';

        // Auto-fill category, subcategory and description using AI summary
        setForm(f => ({
          ...f,
        ...
        toast.success(`✨ Real complaint verified! Auto-selected ${detectedCategory.replace('_', ' ').toUpperCase()} and pre-filled description.`, { duration: 5000 });
        
        // Auto-navigate to Next Step (Description & Location)
        setTimeout(() => {
          setStep(1);
        }, 800);
      } else {
        toast('⚠️ Photo does not appear to show a valid civic complaint or duplicate image detected. You can still proceed manually.', { duration: 6000, icon: '⚠️' });
      }
    } catch (err) {
          ...
          <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
            ...
            {step === 4 && (
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-xl font-extrabold text-slate-900 dark:text-white font-display">Comprehensive Complaint Review</h2>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Please verify all complaint parameters before final dispatch & official registration.</p>
                  </div>
                  <span className="px-3 py-1 rounded-full text-xs font-bold bg-brand-50 dark:bg-brand-950/60 text-brand-600 dark:text-brand-400 border border-brand-200 dark:border-brand-800">
                    Step 5 of 6 • Ready for Dispatch
                  </span>
                </div>

                {/* 1. Category & Routing Overview */}
                <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md space-y-3">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <div className="text-xs font-bold uppercase tracking-wider text-slate-400">Target Category & Subcategory</div>
                        <div className="font-extrabold text-base text-slate-900 dark:text-white">
                          {selectedCategory?.label} <span className="text-slate-400 font-normal">({form.subcategory.replace(/_/g, ' ')})</span>
                        </div>
                      </div>
                    </div>
                    <button type="button" onClick={() => setStep(0)} className="text-xs font-bold text-brand-600 dark:text-brand-400 hover:underline">
                      Edit Category
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs pt-1">
                    <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                      <div className="text-slate-400 font-medium">Assigned Authority Unit</div>
                      <div className="font-bold text-slate-900 dark:text-white mt-0.5 flex items-center gap-1.5">
                        {form.category === 'crime' && '🚔 Police Station (PS)'}
                        {form.category === 'corruption' && '⚖️ Anti-Corruption Bureau (ACB)'}
                        {form.category === 'fire' && '🔥 Fire & Emergency Services'}
                        {form.category === 'hospital' && '🏥 Healthcare & Hospital Administration'}
                        {form.category === 'civic_issue' && '🏛️ Municipal Corporation & Local Body'}
                        {!['crime', 'corruption', 'fire', 'hospital', 'civic_issue'].includes(form.category) && '🏛️ Designated Municipal Authority'}
                      </div>
                    </div>

                    <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
                      <div className="text-slate-400 font-medium">Expected Resolution SLA</div>
                      <div className="font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">
                        3 – 7 Working Days (Auto-escalation after 72 hrs)
                      </div>
                    </div>
                  </div>
                </div>

                {/* 2. AI Vision Telemetry (if AI processed photo) */}
                {aiResult && (
                  <div className="p-4 rounded-2xl bg-gradient-to-r from-indigo-950 via-slate-900 to-purple-950 text-white border border-indigo-700/50 shadow-md space-y-2.5">
                    <div className="flex items-center justify-between text-xs font-bold">
                      <span className="flex items-center gap-1.5 text-indigo-300">
                        <Sparkles size={14} className="text-amber-400 animate-pulse" />
                        AI Multimodal Vision Audit Log ({aiResult.engine || 'Gemini 3.6 Flash'})
                      </span>
                    ...
                    <p className="text-xs text-slate-300 leading-relaxed font-sans">
                      {aiResult.description || aiResult.analysis || aiResult.reason}
                    </p>
                  </div>
                )}
                ...
                <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md space-y-2">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Full Description ({form.description.length} chars)</span>
                    <button type="button" onClick={() => setStep(1)} className="text-xs font-bold text-brand-600 dark:text-brand-400 hover:underline">
                      Edit Text
                    </button>
                  </div>
                  <p className="text-sm text-slate-800 dark:text-slate-200 whitespace-pre-wrap leading-relaxed font-sans bg-slate-50 dark:bg-slate-800/40 p-3.5 rounded-xl border border-slate-100 dark:border-slate-800">
                    {form.description}
                  </p>
                </div>

                {/* 4. Location Telemetry Card */}
                <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md space-y-3">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1.5">
                      <MapPin size={14} className="text-red-500" /> Location & GPS Telemetry
                    </span>
                    <button type="button" onClick={() => setStep(2)} className="text-xs font-bold text-brand-600 dark:text-brand-400 hover:underline">
                      Edit Location
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    <div>
                      <span className="text-slate-400 font-medium">Incident Address:</span>
                      <p className="font-semibold text-slate-900 dark:text-white mt-0.5">{form.location.address || 'Address not specified'}</p>
                    </div>
                    <div>
                      <span className="text-slate-400 font-medium">District / State / Pincode:</span>
                      <p className="font-semibold text-slate-900 dark:text-white mt-0.5 capitalize">
                        {form.location.district || 'Guntur'}, {form.location.state || 'Andhra Pradesh'} {form.location.pincode ? `(${form.location.pincode})` : ''}
                      </p>
                    </div>
                    {form.location.lat && form.location.lng && (
                      <div className="sm:col-span-2 p-2.5 rounded-xl bg-blue-50/70 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300 font-mono text-[11px] flex items-center justify-between">
                        <span>GPS Coordinates: {form.location.lat.toFixed(5)}° N, {form.location.lng.toFixed(5)}° E</span>
                        <span className="font-bold text-emerald-600 dark:text-emerald-400">High Precision GPS Lock</span>
                      </div>
                    )}
                  </div>
                </div>

                {/* 5. Evidence Attachments Preview */}
                <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md space-y-3">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Attached Evidence ({files.length} File{files.length !== 1 ? 's' : ''})</span>
                    <button type="button" onClick={() => setStep(3)} className="text-xs font-bold text-brand-600 dark:text-brand-400 hover:underline">
                      Manage Files
                    </button>
                  </div>
                  ...
                  {files.length > 0 ? (
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                      {files.map((f, idx) => (
                        <div key={idx} className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 text-xs space-y-1">
                          {f.preview ? (
                            <img src={f.preview} alt="" className="w-full h-16 object-cover rounded-lg" />
                          ) : (
                            <div className="w-full h-16 rounded-lg bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-400">
                              <FileImage size={20} />
                            </div>
                          )}
                          <div className="truncate font-semibold text-[11px] text-slate-800 dark:text-slate-200">{f.name}</div>
                          <div className="text-[9px] text-slate-400">{(f.size / (1024 * 1024)).toFixed(2)} MB</div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-xs text-slate-400 italic">No media attachments uploaded. (Optional step)</p>
                  )}
                </div>

                {/* 6. Submission Mode & Privacy Safeguard */}
                <div className="p-4 rounded-2xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    {form.isAnonymous ? <EyeOff size={16} className="text-amber-500" /> : <Eye size={16} className="text-green-500" />}
                    <div>
                      <span className="font-bold text-slate-900 dark:text-white">Submission Mode: </span>
                      <span className="font-semibold text-slate-700 dark:text-slate-300">
                        {form.isAnonymous ? 'Anonymous Mode (Identity Protected)' : 'Standard Registered Submission'}
                      </span>
                    </div>
                  </div>
                  <span className="text-[11px] font-bold text-slate-400 font-mono">ENCRYPTED DISPATCH</span>
                </div>

                <div className="p-3.5 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 flex gap-2 text-xs text-emerald-700 dark:text-emerald-300 font-medium">
                  <CheckCircle size={16} className="flex-shrink-0 mt-0.5" />
                  <span>
                    Upon dispatch, your complaint will be assigned a unique Complaint Tracking ID and ingested into the Smart City Auto-Routing & Escalation Engine.
                  </span>
                </div>
              </div>
            )}
        ...
        {step < STEPS.length - 1 && (
          <div className="flex justify-between mt-8">
            ...
            {step === STEPS.length - 2 ? (
                ...
                {submitting ? 'Submitting Report...' : duplicateLoading ? 'Checking Duplicates...' : 'Confirm & Submit'}
              </button>
            ) : (
