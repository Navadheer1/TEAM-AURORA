    ...
    try:
        ...
        if db:
            ...
            for doc in docs:
                ...
                if lat is not None and lng is not None:
                    ...
                    if dist is not None and dist > CANDIDATE_RADIUS_METERS:
                        continue
                candidates.append({'id': doc.id, 'data': data, 'distance': dist})
        else:
            # Fallback to Django SQLite model
            from api.models import Complaint
            qs = Complaint.objects.filter(status__in=['pending', 'assigned', 'in_progress'])
            if cat:
            ...
            for item in qs[:100]:
                ...
                dist = None
                if lat is not None and lng is not None:
                    c_loc = item.location or {}
                    c_lat = c_loc.get('lat')
                    c_lng = c_loc.get('lng')
                    dist = haversine_distance(lat, lng, c_lat, c_lng)
                    if dist is not None and dist > CANDIDATE_RADIUS_METERS:
                        continue
                candidates.append({'id': item.id, 'data': data, 'distance': dist})
