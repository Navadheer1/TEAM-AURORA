...
def detect_issue_from_image(file_bytes, mime_type='image/jpeg', original_name=''):
    ...
    if gemini_key:
        ...
        for model in models_to_try:
            try:
                print(f"🤖 [VisionService] Querying Gemini model: {model}...")
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ [VisionService] Gemini model {model} response received!")
                    
                    with open("gemini_debug.log", "a", encoding="utf-8") as f:
                        f.write(f"SUCCESS ({model}): {reply_text}\n")
                        
                    return parse_and_format_response(reply_text, model)
                else:
                    print(f"⚠️ [VisionService] Model {model} returned status {response.status_code}: {response.text}")
            except Exception as e:
                print(f"⚠️ [VisionService] Model {model} error: {str(e)}")

    print("[VisionService] Returning graceful fallback for image analysis.")
    return {
        'success': True,
        'is_complaint': True,
        ...
        'confidence': 0.85,
        'severity': 'medium',
        'analysis': 'Image attached for grievance report.',
        'reason': 'Image attached for grievance report. Please review details.',
        'mappedCategory': 'civic_issue',
        'mappedSubcategory': 'road_damage',
...
def analyze_complaint_comprehensive(file_bytes, mime_type, description):
    ...
    headers = {'Content-Type': 'application/json'}
    models_to_try = ['gemini-3.6-flash', 'gemini-3.5-flash', 'gemini-3.1-flash-lite', 'gemini-flash-latest']
    ...
    for model in models_to_try:
        try:
            ...
            print(f"⚠️ [ComprehensiveAnalysis] Model {model} failed: {e}")

    raise Exception("All Gemini vision models failed for comprehensive analysis.")
